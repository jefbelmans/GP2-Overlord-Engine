	...
	void onContactModify(PxContactModifyPair* const pairs, PxU32 count)
	{
		for (PxU32 i = 0; i < count; i++)
		{
			...
			for (PxU32 j = 0; j < 2; j++)
			{
				...
				if (actor->userData && (static_cast<vehicle::ActorUserData*>(actor->userData))->vehicle)
				{
					const PxVehicleWheels* vehicle = (static_cast<vehicle::ActorUserData*>(actor->userData))->vehicle;
					PX_ASSERT(vehicle->getRigidDynamicActor() == actors[j]);

					const PxShape* shape = shapes[j];
					if (shape->userData && (static_cast<vehicle::ShapeUserData*>(shape->userData))->isWheel)
					{
						const PxU32 wheelId = (static_cast<vehicle::ShapeUserData*>(shape->userData))->wheelId;
